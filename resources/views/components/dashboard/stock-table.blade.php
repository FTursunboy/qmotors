<div class="d-flex mb-2">
    ...
    <a href="{{ route('stock.create') }}" class="ml-auto btn btn-primary">Добавить</a>
</div>
<x-dashboard.default-table length="{{ count($list) }}">
    <x-slot name="header">
        ...
        <th>
            <x-column-order-caret column="description">Описание</x-column-order-caret>
        </th>
        <th>
    ...
    <x-slot name="body">
        ...
        <tr>
            <td><a href="{{ route('stock.show', $item->id) }}">{{ $item->id }}</a></td>
            <td>{{ $item->title }}</td>
            <td>{{ $item->subtitle }}</td>
            ...
            <td>
                <div class="d-flex float-right">
                    <a class="btn btn-info mr-2" href="{{ route('stock.show', $item->id) }}"><i
                            class="fa fa-eye"></i></a>
                    <a href="{{ route('stock.edit', $item->id) }}" class="btn btn-primary mr-2"><i
                            class="fa fa-pen"></i></a>
